...
Enable Kiro to assist with cloud security assessments across AWS, Azure, and GCP, container and Kubernetes security hardening, Infrastructure as Code (Terraform, CloudFormation, Kubernetes manifests) scanning, and cloud compliance reporting against CIS Benchmarks and major frameworks.
...
- `AWS CLI` â€” AWS auditing and management
- `ScoutSuite` â€” Multi-cloud security audit
- `Prowler` â€” AWS/Azure/GCP security assessment
- `Checkov` â€” IaC static analysis
- `tfsec` â€” Terraform security scanner
- `Trivy` â€” Container and IaC vulnerability scanner
- `kube-bench` â€” CIS Kubernetes Benchmark
- `Falco` â€” Container runtime security
...
[ ] Password policy: min 14 chars, complexity, rotation â‰¤90 days
...
Networking:
[ ] No security groups with 0.0.0.0/0 â†’ port 22 (SSH)
[ ] No security groups with 0.0.0.0/0 â†’ port 3389 (RDP)
...
Risk: All objects publicly readable â€” potential data breach
...
Kiro reads and analyzes the Dockerfile directly:

**Dockerfile Security Review:**
```dockerfile
# BEFORE (insecure)
FROM ubuntu:latest           # â† Never use latest
RUN apt-get install -y curl  # â† Install what you need during build
COPY . .                     # â† Copies everything including .env files
RUN chmod 777 /app           # â† World-writable is dangerous
CMD ["./app"]                # â† Runs as root by default
EXPOSE 0-65535               # â† Never expose all ports
...
Kiro reads the IaC files directly and identifies issues:
...
resource "aws_security_group_rule" "bad" {
  ...
  to_port     = 65535
  protocol    = "tcp"
  cidr_blocks = ["0.0.0.0/0"]    # â† OPEN TO INTERNET!
}
```
...
|-----------|---------------|
| Cloud assets discovered via recon | â† Skill 01 (Recon & OSINT) |
| Cloud vulnerabilities for CSOC alerts | â†’ Skill 11 (CSOC Automation) |
| Implement cloud hardening recommendations | â†’ Skill 15 (Blue Team Defense) |

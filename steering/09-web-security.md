...
Enable Kiro to assist with comprehensive web application security assessments covering OWASP Top 10, injection testing, API security, authentication analysis, and client-side security. Kiro analyzes application behavior, generates test payloads, reviews source code, and produces structured vulnerability reports.
...
**Recommended tools:**
- `Burp Suite Community/Pro` â€” Web proxy and scanner
- `OWASP ZAP` â€” Open-source web scanner
- `sqlmap` â€” Automated SQL injection (authorized use only)
- `Nikto` â€” Web server scanner
- `ffuf / feroxbuster` â€” Web fuzzer
- `jwt_tool` â€” JWT analysis and manipulation
...
| # | Vulnerability | Kiro's Assessment Approach |
|---|--------------|------------------------------|
...
**Input Discovery â€” Map all injection points:**
```
GET/POST parameters
...
```
Step 1: Detection â€” Test for error-based confirmation
  ' â†’ SQL error = likely vulnerable
  ' OR '1'='1 â†’ true condition
  ' OR '1'='2 â†’ false condition
  ' AND SLEEP(5)-- - â†’ time delay = blind SQLi

Step 2: Fingerprint the database
  ' AND 1=CONVERT(int,@@version)-- - â†’ MSSQL version
  ' AND 1=1 UNION SELECT @@version-- - â†’ MySQL
  ' AND 1=(SELECT 1 FROM dual)-- - â†’ Oracle
...
Step 1: Find reflection points
  Input: test123  â†’  Search in source for "test123"
  What HTML context is it in?
    â€¢ HTML content: <p>test123</p>
    â€¢ Attribute: <input value="test123">
    â€¢ JavaScript: var x = "test123";
    â€¢ URL: href="test123"
...
{{7*7}}           â†’ 49 (Jinja2, Twig, Freemarker)
${7*7}            â†’ 49 (FreeMarker, Velocity, Mako)
<%= 7*7 %>        â†’ 49 (ERB/Ruby)
#{7*7}            â†’ 49 (Pug/Jade)

# Jinja2 (Python Flask) â€” confirm RCE path
{{config}}
...
Authorization:
[ ] BOLA â€” Change object ID to access other users' data
  GET /api/users/123/orders â†’ try /api/users/124/orders
[ ] Mass assignment â€” POST with extra privileged fields
  {"username":"user","role":"admin","isAdmin":true}
[ ] Function-level access â€” access admin endpoints as regular user
  GET /api/admin/users (as non-admin)

Input Validation:
[ ] SQL injection in API parameters
[ ] JSON injection in body
[ ] Rate limiting â€” rapid requests cause DoS or bypass?
[ ] Request body schema â€” what happens with extra/unexpected fields?
...
```graphql
# Introspection â€” should be disabled in production
...
```
Entropy check:
[ ] Token length sufficient? (â‰¥128 bits recommended)
...
[ ] Scope validation â€” can client request broader scopes than expected?
```
...
**CWE:** [CWE-ID â€” CWE Name]
**OWASP:** [A0X â€” Category]
...
### Impact
[Business and technical impact â€” data exposure, account takeover, RCE, etc.]
...
|-----------|---------------|
| Web apps discovered during recon | â† Skill 01 (Recon & OSINT) |
| Vulnerable components identified | â†’ Skill 02 (Vulnerability Scanner) |
| Develop PoC for confirmed vuln | â†’ Skill 03 (Exploit Development) |
| Generate CSOC alerts for findings | â†’ Skill 11 (CSOC Automation) |

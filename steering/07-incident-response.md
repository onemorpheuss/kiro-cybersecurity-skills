...
Enable Kiro to assist with structured incident response operations following NIST SP 800-61 and the SANS PICERL framework. Kiro generates IR playbooks, guides evidence collection with chain of custody, constructs forensic timelines, interprets memory forensics output, and produces post-incident reports.
...
- `plaso / log2timeline` â€” Supertimeline generation
- `KAPE` â€” Evidence collection (Windows)
- `Velociraptor` â€” Enterprise-scale endpoint forensics
- `FTK Imager` â€” Forensic imaging (Windows)
- `dd / dcfldd / dc3dd` â€” Disk imaging (Linux)
...
Kiro generates detailed, role-assigned playbooks in this structure:
...
- [ ] Identify Patient Zero â€” first encrypted system
...
**Order of Volatility (most volatile â†’ least volatile):**
```
1. CPU registers and cache
2. Routing tables, ARP cache, process table
3. Memory (RAM) â€” ALWAYS capture first
...
| Chain of custody | Analyst â†’ Evidence Locker â†’ Lab |
...
2. **Normalize to UTC** â€” Confirm system timezone before conversion
...
```markdown
## Incident Timeline â€” [Incident ID]

[T-72h] 2025-05-25 09:15 UTC â€” DELIVERY
...
[T-48h] 2025-05-26 14:22 UTC â€” EXECUTION
  User executed attachment: Event 4688 (process creation)
  Parent: outlook.exe â†’ Child: powershell.exe -enc [base64]

[T-48h] 2025-05-26 14:22 UTC â€” C2 ESTABLISHED
  Outbound connection: 203.x.x.x:443 (beacon_interval: 60s)
  DNS query: malicious-c2.evil.com â†’ 203.x.x.x

[T-24h] 2025-05-27 02:00 UTC â€” LATERAL MOVEMENT
  PsExec from WORKSTATION01 to SERVER02 (admin$)
  Event 4624 (login type 3) on SERVER02 from WORKSTATION01

[T-2h]  2025-05-27 12:30 UTC â€” DATA EXFILTRATION
  Large POST request (450MB) to dropbox-like service

[T-0h]  2025-05-28 14:00 UTC â€” DETECTION
...
```markdown
# Post-Incident Report â€” [Incident ID]
...
- **GDPR:** [Required if EU personal data â€” 72-hour notification to DPA]
- **HIPAA:** [Required if PHI â€” notify HHS within 60 days]
- **PCI-DSS:** [Required if cardholder data â€” notify card brands immediately]
...
|-----------|---------------|
| Alert received from SOC â†’ activate IR | â† Skill 11 (CSOC Automation) |
| Malware samples collected â†’ analyze | â†’ Skill 05 (Malware Analysis) |
| IOCs extracted â†’ hunt in environment | â†’ Skill 06 (Threat Hunting) |
| Log deep-dive needed | â†’ Skill 12 (Log Analysis) |

---

## References

- [NIST SP 800-61 Rev. 2 â€” Computer Security Incident Handling](https://csrc.nist.gov/publications/detail/sp/800-61/rev-2/final)

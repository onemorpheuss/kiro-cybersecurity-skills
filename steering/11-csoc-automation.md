...
Enable Kiro to assist Cyber Security Operations Center (CSOC) teams with structured alert triage, automated playbook creation, escalation workflow design, shift handover reporting, and SOC metrics analysis. Kiro produces operational artifacts that analysts can execute directly or adapt to their SOAR platforms.
...
- `Splunk SOAR` â€” Playbook automation
- `Palo Alto XSOAR` â€” SOAR platform
- `TheHive` â€” Open-source IR platform
- `ServiceNow` â€” ITSM ticketing
- `PagerDuty / OpsGenie` â€” Alerting and on-call
...
|----------------|-------------------|--------------------|-----|
| High | Critical | Immediate escalation to Tier 2/3 â€” declare incident | 15 min |
...
**Alert ID:** [ID]
**Alert Type:** [Type â€” e.g., Brute Force Login]
...
- Known threat actor: [Yes/No â€” if yes, attribution]
...
tasks:
  ...
  - id: "4-contain"
    ...
    actions:
      ...
      - If user clicked: isolate endpoint (â†’ IR Playbook)
  ...
  - id: "5-notify"
    ...
    templates:
      user_notification: |
        Subject: Action Required â€” Phishing Email in Your Inbox
...
|-------|--------------|---------------|------|
| P1 â€” Critical | 9.0â€“10.0 or system breach | 15 minutes | Tier 3 + IR Lead + Management |
| P2 â€” High | 7.0â€“8.9 or data at risk | 30 minutes | Tier 2 + Tier 3 standby |
| P3 â€” Medium | 4.0â€“6.9 | 2 hours | Tier 1 â†’ Tier 2 if unresolved |
| P4 â€” Low | 0.1â€“3.9 | 8 hours | Tier 1 |

### Escalation Paths
```
P1 Incident:
  Tier 1 Analyst â†’ [immediately] â†’ Tier 3 Lead (call)
  Tier 3 Lead â†’ [within 5 min] â†’ SOC Manager (call)
  SOC Manager â†’ [within 15 min] â†’ CISO + Legal (if data breach)

P2 Incident:
  Tier 1 Analyst â†’ [after 30 min unresolved] â†’ Tier 2 Analyst (ticket escalation)
  Tier 2 Analyst â†’ [after 2 hours unresolved] â†’ Tier 3 Lead (chat notification)

Out-of-hours escalation:
  On-call Tier 2 via PagerDuty â†’ 15 min acknowledge â†’ escalate to Tier 3
```
...
```
ðŸš¨ *P1 SECURITY INCIDENT DECLARED* ðŸš¨
*Type:* {{incident_type}}
...
```
Subject: [P{{severity}}] Security Incident â€” {{incident_type}} â€” {{ticket_id}}
...
- Increased scan activity from 198.51.100.0/24 â€” monitoring trend
- User john.doe@company.com multiple failed VPN logins â€” watching for successful auth

## Tool/System Issues
- Splunk indexer latency 45 min this shift â€” events delayed, may affect investigation timing
- EDR console intermittently slow â€” reported to IT ops (ticket: OPS-4421)

## Recommended Actions for Next Shift
1. Follow up on INC-2025-089: EDR report expected from endpoint team by 08:00
2. Review john.doe@company.com VPN activity â€” if successful login from new location, triage
...
|--------|---------|--------|
| **MTTD** (Mean Time to Detect) | Avg(detection_time âˆ’ attack_start_time) | < 1 hour |
| **MTTR** (Mean Time to Respond) | Avg(response_complete âˆ’ detection_time) | < 4 hours |
| **MTTC** (Mean Time to Contain) | Avg(contain_time âˆ’ detection_time) | < 1 hour |
| **False Positive Rate** | FP / (FP + TP) Ã— 100 | < 20% |
...
```markdown
## SOC Monthly Metrics Report â€” May 2025
...
- MTTD: 23 minutes (â†“ from 31 min last month) âœ“
- MTTR: 3.2 hours (â†‘ from 2.8 hours) âš 
- SLA Compliance: 96.1% âœ“
...
2. MTTR increased due to EDR tool latency â€” investigate performance
```
...
|-----------|---------------|
| Alert requires deep investigation | â†’ Skill 07 (Incident Response) |
| Log deep-dive for alert context | â†’ Skill 12 (Log Analysis) |
| Threat hunt based on alert patterns | â†’ Skill 06 (Threat Hunting) |
| All detection skills feed alerts here | â† All detection skills |

...
Enable Kiro to assist defenders with comprehensive security hardening, detection rule engineering, security baseline establishment, patch management, and security architecture review. Kiro directly analyzes provided configurations, scripts, and system state â€” then produces specific hardening commands, detection rules, and improvement plans.
...
- `Sysmon` â€” Windows endpoint telemetry (SwiftOnSecurity config recommended)
- `auditd` â€” Linux audit daemon
- `Lynis` â€” Linux security auditing tool
- `OpenSCAP / oscap` â€” CIS/STIG compliance scanning
- `fail2ban` â€” SSH and service brute-force protection
- `CIS-CAT` â€” CIS Benchmark compliance tool
...
Kiro produces specific commands ready to run.

#### SSH Hardening
```bash
# /etc/ssh/sshd_config â€” Secure SSH configuration
...
# iptables â€” manual approach for fine-grained control
...
**PowerShell â€” Immediate Hardening Commands:**
```powershell
...
# Disable NBT-NS (NetBIOS Name Service â€” used in Responder attacks)
...
# Set via Group Policy: Computer Configuration â†’ Administrative Templates â†’ 
...
# (Or configure via Group Policy â†’ Computer Config â†’ Windows Settings â†’ Security Settings â†’ Advanced Audit Policy)
...
Kiro produces complete, ready-to-deploy detection rules.
...
Step 1: Define what you're detecting
  ...
  - Which ATT&CK technique? (T1059.001 â€” PowerShell)
...
status: stable
description: Detects Office applications spawning script interpreters â€” common in macro-based initial access
references:
...
```markdown
## Security Baseline â€” [System Type] â€” [Environment]

### Normal Behavior Profiles

**Authentication Baseline:**
- Admin accounts: Only log in during business hours (08:00â€“18:00 local)
...
- System processes: Parent process matches expected (e.g., services.exe â†’ svchost.exe)
...
|------|---------------|-------------|---------|-----|
| 9.0â€“10.0 | Remote, no auth | Yes | P1 â€” Emergency | 24 hours |
| 9.0â€“10.0 | Remote, no auth | No | P1 â€” Critical | 48 hours |
| 7.0â€“8.9 | Remote | Any | P2 â€” High | 7 days |
| 4.0â€“6.9 | Local | Any | P3 â€” Medium | 30 days |
| 0.1â€“3.9 | Any | No | P4 â€” Low | 90 days |
| 0.0 | N/A | N/A | P5 â€” Info | Next cycle |
...
3. TEST in non-production environment (Dev â†’ QA â†’ Staging)
4. PLAN rollout window (off-peak, with rollback procedure)
5. DEPLOY to production (phased: 10% â†’ 50% â†’ 100%)
...
```
Layer 1 â€” Perimeter Defense
  â”œâ”€â”€ External firewall / WAF
  â”œâ”€â”€ IDS/IPS (Suricata/Snort)
  â”œâ”€â”€ DDoS protection
  â””â”€â”€ DNS filtering

Layer 2 â€” Network Segmentation
  â”œâ”€â”€ VLANs: Servers / Users / IoT / Guests / Management
  â”œâ”€â”€ East-west traffic controls
  â”œâ”€â”€ Micro-segmentation for critical systems
  â””â”€â”€ Network access control (802.1X)

Layer 3 â€” Endpoint Defense
  â”œâ”€â”€ EDR (CrowdStrike, SentinelOne, Defender for Endpoint)
  â”œâ”€â”€ AV / NGAV
  â”œâ”€â”€ Host firewall
  â””â”€â”€ Application control (AppLocker / WDAC)

Layer 4 â€” Identity & Access Management
  â”œâ”€â”€ MFA for all users
  â”œâ”€â”€ Privileged Access Management (PAM)
  â”œâ”€â”€ JIT / JEA (Just-in-Time, Just-Enough-Admin)
  â””â”€â”€ Zero Trust Network Access (ZTNA)

Layer 5 â€” Data Protection
  â”œâ”€â”€ Encryption at rest (BitLocker, dm-crypt)
  â”œâ”€â”€ Encryption in transit (TLS 1.2/1.3)
  â”œâ”€â”€ Data Loss Prevention (DLP)
  â””â”€â”€ Backup with integrity verification

Layer 6 â€” Detection & Response
  â”œâ”€â”€ SIEM (log aggregation + correlation)
  â”œâ”€â”€ SOAR (automated response)
...
[ ] Privileged accounts isolated (PAW â€” Privileged Access Workstations)
...
### P1 â€” Immediate (48 hours)
- [ ] Disable LLMNR and NBT-NS (used for initial credential capture)
  â†’ `Set-ItemProperty HKLM:\... EnableMulticast 0`
...
|-----------|---------------|
| Red team findings â†’ create remediation plan | â† Skill 14 (Red Team Operations) |
| Detection rules â†’ deploy to SIEM | â†’ Skill 12 (Log Analysis) |
| New detection rules â†’ add to CSOC | â†’ Skill 11 (CSOC Automation) |
| Vulnerability scanner findings â†’ patch plan | â† Skill 02 (Vulnerability Scanner) |
| Threat hunt gaps â†’ improve detection coverage | â† Skill 06 (Threat Hunting) |

...
Enable Kiro to assist with cryptographic security assessments including SSL/TLS configuration auditing, cipher suite analysis and recommendation, hash algorithm identification, encryption implementation code review, key management evaluation, and detection of cryptographic vulnerabilities. Kiro directly analyzes provided configurations and code.
...
**Recommended tools:**
- `sslyze` â€” Python TLS scanner
- `testssl.sh` â€” Comprehensive TLS testing
- `openssl` â€” Command-line TLS operations
- `Wireshark` â€” TLS traffic analysis
- `certbot` â€” Certificate management
...
| TLS 1.0 | Deprecated (PCI-DSS violation) | Disable â€” BEAST, POODLE |
...
**TLS 1.3 Cipher Suites (All Secure â€” Use These):**
...
|-------------|--------|-------|
| ECDHE-ECDSA-AES256-GCM-SHA384 | A+ | Perfect â€” AEAD, PFS |
| ECDHE-RSA-AES256-GCM-SHA384 | A+ | Perfect â€” AEAD, PFS |
| ECDHE-RSA-AES128-GCM-SHA256 | A | Good â€” AEAD, PFS |
| DHE-RSA-AES256-GCM-SHA384 | A | Good â€” if DHE â‰¥ 2048-bit |
| AES256-GCM-SHA384 | B | No forward secrecy |
| ECDHE-RSA-AES256-SHA384 | B | CBC mode (timing attacks) |
| RC4-SHA | F | RC4 broken â€” never use |
...
|----------------------|-----------|----------------|
| 32 hex chars | MD5 | Broken â€” collision attacks exist |
| 40 hex chars | SHA-1 | Deprecated â€” SHAttered collision |
...
| `$apr1$...` | MD5-APR (Apache) | Weak â€” only 1000 iterations |
| `$1$...` | MD5-crypt | Broken |
...
| `pbkdf2_sha256$...` | PBKDF2-SHA256 | Good if iterations â‰¥ 260,000 |

**Password Hash Security Assessment:**

```
bcrypt:
  $2b$10$... â†’ cost factor 10 â†’ ~100ms/hash â†’ ACCEPTABLE
  $2b$12$... â†’ cost factor 12 â†’ ~400ms/hash â†’ GOOD
  $2b$14$... â†’ cost factor 14 â†’ ~1.6s/hash  â†’ STRONG
...
Avoid:
  Plain MD5/SHA-1 â†’ instantly cracked
  bcrypt cost < 10 â†’ too fast for modern hardware
  No salt â†’ rainbow table attack
  SHA-256 without KDF â†’ GPU-crackable
```
...
Kiro reads the code directly and flags these patterns:

**Python Crypto Anti-Patterns:**
```python
# INSECURE: Hardcoded encryption key
key = b"mysecretkey12345"  # â† NEVER hardcode keys
cipher = AES.new(key, AES.MODE_ECB)  # â† ECB mode is insecure

# INSECURE: ECB mode (produces identical ciphertext for identical blocks)
from Crypto.Cipher import AES
cipher = AES.new(key, AES.MODE_ECB)  # â† Pattern detection attacks possible

# INSECURE: Static/reused IV
iv = b"\x00" * 16  # â† Static IV allows pattern detection
cipher = AES.new(key, AES.MODE_CBC, iv)  # â† Reusing IV with same key = CRITICAL

# INSECURE: Weak hash for passwords
import hashlib
password_hash = hashlib.md5(password.encode()).hexdigest()  # â† GPU-crackable

# INSECURE: Trusting certificate errors
import ssl
ssl._create_default_https_context = ssl._create_unverified_context  # â† MitM possible
requests.get(url, verify=False)  # â† Never do this in production
```
...
[ ] Tokens and OTPs have sufficient entropy (â‰¥128 bits)
```
...
- **ML-KEM** (CRYSTALS-Kyber) â€” Key encapsulation mechanism (replaces RSA/ECDH for key exchange)
- **ML-DSA** (CRYSTALS-Dilithium) â€” Digital signature (replaces RSA/ECDSA)
- **SLH-DSA** (SPHINCS+) â€” Stateless hash-based signature (conservative fallback)
...
|-----------|---------------|
| TLS audit for web application | â† Skill 09 (Web Security) |
| Cloud service encryption assessment | â† Skill 10 (Cloud Security) |
| Weak crypto findings â†’ hardening recommendations | â†’ Skill 15 (Blue Team Defense) |
| Crypto flaws â†’ vulnerability report | â†’ Skill 02 (Vulnerability Scanner) |
...
- [NIST SP 800-52 Rev. 2 â€” TLS Guidelines](https://csrc.nist.gov/publications/detail/sp/800-52/rev-2/final)

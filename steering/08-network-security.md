...
Enable Kiro to assist with network security operations including traffic analysis from PCAP files, IDS/IPS rule authoring for Snort and Suricata, firewall rule auditing, network anomaly detection, and network architecture security reviews.
...
**Recommended tools:**
- `Wireshark / tshark` â€” Packet capture and GUI analysis
- `Suricata` â€” Modern IDS/IPS engine
- `Snort 3` â€” Classic IDS/IPS engine
- `Zeek (Bro)` â€” Network analysis and scripting framework
- `tcpdump` â€” Command-line packet capture
- `NetworkMiner` â€” PCAP artifact extraction
- `nmap` â€” Network scanning and discovery
...
```
[ ] Protocol distribution â€” any unexpected protocols?
[ ] Top talkers â€” unusual source/destination combinations
[ ] DNS analysis â€” DGA domains, unusually long queries, high volume
[ ] HTTP analysis â€” suspicious user agents, unusual methods, encoded data
[ ] TLS analysis â€” invalid certificates, unusual SNI, cert fingerprints
[ ] ICMP analysis â€” large payloads (tunneling), ping sweeps
[ ] SMB analysis â€” authentication attempts, file access patterns
[ ] Data volume â€” large uploads (exfiltration?), irregular transfer sizes
[ ] Timing analysis â€” regular interval beaconing patterns
```
...
```snort
# Snort 3 format â€” note different syntax from Snort 2
...
âœ— Inbound: 0.0.0.0/0 â†’ Port 22 (SSH open to internet)
âœ— Inbound: 0.0.0.0/0 â†’ Port 3389 (RDP open to internet)
âœ— Inbound: 0.0.0.0/0 â†’ Port 0-65535 (any port from internet)
âœ— Outbound: 0.0.0.0/0 â†’ Port 0-65535 (unrestricted outbound)

Rules that are acceptable:
âœ“ Inbound: 0.0.0.0/0 â†’ Port 443 (HTTPS for public web service)
âœ“ Inbound: 10.0.0.0/8 â†’ Port 22 (SSH from internal VPC only)
âœ“ Inbound: [known IP] â†’ Port 22 (SSH from jump box)
```

**Firewall Audit Report Template:**
```markdown
## Firewall Audit Report â€” [Device/Platform]
...
Zone Model (most to least trusted):
  Internal (Core) â†’ DMZ â†’ Internet
...
|-----------|---------------|
| Discovered services from recon | â† Skill 01 (Recon & OSINT) |
| Network IOCs for threat correlation | â†’ Skill 06 (Threat Hunting) |
| Network evidence for IR timeline | â†’ Skill 07 (Incident Response) |
| Automate alert responses | â†’ Skill 11 (CSOC Automation) |

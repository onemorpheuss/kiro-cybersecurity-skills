...
Enable Kiro to assist threat hunters with proactive threat detection, IOC extraction and normalization, MITRE ATT&CK mapping, hunt hypothesis generation, and converting threat intelligence into actionable detection rules across all major SIEM platforms.
...
**Optional platforms:**
- MISP â€” Threat intelligence sharing platform
- OpenCTI â€” Threat intelligence platform
- YARA â€” Pattern matching (â†’ Skill 05)
- Sigma CLI â€” Rule conversion tool
...
Kiro performs these extraction steps:
...
2. **Defang extracted indicators** â€” refang before use:
   - `hxxp://` â†’ `http://`
   - `[.]` â†’ `.`
   - `[at]` â†’ `@`
   - `[:]` â†’ `:`
...
```json
{
  ...
  "name": "Malicious IP â€” C2 Infrastructure",
...
2. Map to specific Tactic â†’ Technique â†’ Sub-technique (T1059 â†’ T1059.001)
...
```json
{
  "name": "Threat Hunt Layer â€” [Threat Actor/Campaign]",
...
```markdown
## Hunt Hypothesis â€” [ID]: [Short Name]
...
**Rationale:**
[Why this threat is relevant to this organization â€” industry, exposure, recent news]
...
**Success Criteria:**
- POSITIVE: We find evidence of the technique â†’ escalate to IR (Skill 07)
- NEGATIVE: No evidence after thorough search â†’ document as cleared hunt
- INCONCLUSIVE: Insufficient data â†’ identify logging gaps
...
```spl
// T1059.001 â€” PowerShell Execution with suspicious flags
...
// T1547.001 â€” Registry Run Key Persistence
...
// T1021.002 â€” Lateral Movement via SMB Admin Shares
...
```kql
// T1110.001 â€” Brute Force Login Attempt
SecurityEvent
| where EventID == 4625
...
// T1566.001 â€” Spearphishing with attachment
...
1. Cross-reference infrastructure across known campaigns:
   - Same registrar + similar registration dates â†’ likely related infrastructure
   - IP hosting multiple C2 domains â†’ infrastructure cluster
   - Certificate SAN fields â†’ reveal connected domains
...
3. Generate Threat Assessment:
   ```markdown
   ## Threat Assessment â€” [Campaign Name]
...
|-----------|---------------|
| IOCs from malware samples | â† Skill 05 (Malware Analysis) |
| IOCs from IR engagement | â† Skill 07 (Incident Response) |
| Feed hunting queries to SIEM | â†’ Skill 12 (Log Analysis) |
| Generate detection rules | â†’ Skill 15 (Blue Team Defense) |
| Automate response to findings | â†’ Skill 11 (CSOC Automation) |

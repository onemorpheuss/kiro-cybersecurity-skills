...
Enable Kiro to assist authorized red team operators with engagement planning, C2 infrastructure design, attack methodology guidance, lateral movement strategy, OPSEC planning, and comprehensive reporting. Every workflow requires confirmed written authorization.

> **CRITICAL â€” AUTHORIZATION GATE**: Red team assistance requires explicit authorization confirmation before proceeding. Kiro will ask for authorization context and will not assist with active attack planning without it.
>
> **Authorized contexts:**
...
- `Cobalt Strike / Sliver / Havoc` â€” C2 frameworks
- `Metasploit` â€” Exploitation framework
- `BloodHound / SharpHound` â€” AD attack path analysis
- `Impacket` â€” Network protocol tools
- `CrackMapExec / NetExec` â€” AD enumeration
- `Responder` â€” LLMNR/NBT-NS poisoning
- `Mimikatz` â€” Credential access (Windows)

---

## Authorization Verification

**Before any operational planning, Kiro asks:**
...
1. What is the engagement type?
   ...
   â–¡ Internal network assessment
   â–¡ Red team / adversary simulation
   â–¡ Social engineering assessment
   â–¡ Physical security assessment
   â–¡ CTF competition

2. What is your authorization basis?
   â–¡ Signed SOW / contract with target organization
   â–¡ Internal role (IT/Security team testing own systems)
   â–¡ Bug bounty â€” [program name]
   â–¡ CTF â€” [platform and challenge name]
...
4. Who is the target organization's security point of contact?
   (For deconfliction â€” required for IR-level engagements)
...
1. Reconnaissance â†’ OSINT, subdomain enum (Skill 01)
2. Initial Access â†’ Phishing / external vuln exploitation
3. Execution â†’ PowerShell / LOLBins / custom implant
4. Persistence â†’ Registry / service / scheduled task
5. Privilege Escalation â†’ Local privesc â†’ Domain Admin
6. Defense Evasion â†’ Process injection / AMSI bypass
7. Credential Access â†’ LSASS / Kerberoasting / DCSync
8. Lateral Movement â†’ PSExec / WMI / RDP
9. Collection â†’ Identify critical data
10. Exfiltration â†’ Staged transfer to simulated C2
...
2. Call [POC] at [Phone] â€” available 24/7
...
```
[Team Server] â† (internal/VPN only) â†’ [Redirector 1 (HTTPS)] â† â†’ [Beacon]
                                    â†’ [Redirector 2 (DNS)]   â† â†’ [Beacon]
                                    â†’ [Backup Redirector]
```

**Infrastructure Components:**

1. **Team Server** â€” Never directly exposed to internet; VPN access only
2. **Redirectors** â€” Cloud VPS instances (AWS/Azure/GCP) that proxy C2 traffic
3. **C2 Channels** â€” HTTPS (primary), DNS (backup), WebSocket (evasive)
4. **Domain Selection** â€” Aged domains, categorized (business/tech), valid cert

**Redirector Setup (Apache mod_rewrite):**
```apache
# Redirect only Cobalt Strike beacon traffic to team server
# Everything else â†’ sends to legitimate domain (blend in)
RewriteEngine On
...
[ ] Domain aged â‰¥30 days before operation
...
|-----------|-----------|------|-------------|
| Kerberoasting | T1558.003 | Rubeus, Impacket | Request TGS for SPNs â†’ crack offline |
...
```bash
# Using Impacket (Linux â†’ Windows domain)
...
1. **Scope confirmation** â€” What targets are authorized? What is prohibited?
2. **Pretext development** â€” What scenario is believable for this organization?
   - IT help desk password reset
   - Finance: invoice / payment confirmation
   - HR: benefits enrollment
   - Executive: board communication
3. **Infrastructure setup** â€” Phishing domain, email server, landing page
4. **Landing page** â€” Credential harvester or payload delivery
5. **Tracking** â€” Click tracking, credential capture, payload execution
6. **Reporting metrics** â€” Click rate, credential submission rate, report rate

**Pretext Template (for authorized campaigns):**
```
Subject: Action Required: IT Security Policy Update â€” Password Reset Required
...
| Domain Admin | T+12h | Kerberoasting â†’ hash cracked | No |
...
|-----------|---------------|
| Initial recon phase | â†’ Skill 01 (Recon & OSINT) |
| Exploit confirmed vulnerabilities | â† Skill 03 (Exploit Development) |
| Provide findings for defensive improvements | â†’ Skill 15 (Blue Team Defense) |
| Test SOC detection capabilities | â†’ Skill 11 (CSOC Automation) |

...
Enable Kiro to assist with security log analysis across all major platforms. Kiro directly parses and analyzes log samples provided by the user, builds SIEM queries for any platform, creates Sigma rules for portable detection, develops correlation rules, and identifies anomalous patterns in log data.
...
**Platform tools:**
- `Splunk` â€” Splunk Web, SPL, and SOAR
- `Elastic Stack` â€” Kibana, KQL, EQL
- `Microsoft Sentinel` â€” KQL, Workbooks
- `IBM QRadar` â€” AQL, Rules
- `Sigma` â€” Platform-agnostic rule format
- `python-evtx` â€” Parse Windows .evtx files without Windows
...
Kiro directly reads and analyzes logs to extract security-relevant events.

**Windows Event Log â€” Critical Event IDs:**

| Event ID | Log | Description |
|----------|-----|-------------|
| 4624 | Security | Successful logon â€” Logon Type 3 (network) is interesting |
| 4625 | Security | Failed logon â€” track source IP for brute force |
| 4648 | Security | Logon with explicit credentials (RunAs) |
| 4688 | Security | New process created â€” needs CommandLine auditing enabled |
...
**Linux Log Analysis â€” Key Patterns:**
```bash
# Failed SSH logins
...
#### Splunk SPL â€” Attack Pattern Queries
...
#### Microsoft Sentinel KQL â€” Queries
...
#### Elastic EQL â€” Sequence Detection
...
```python
# Kiro's approach to analyzing log data for anomalies:
import pandas as pd
...
```markdown
## Correlation Rule: Brute Force â†’ Successful Login â†’ Lateral Movement

**Trigger:** 
  Event 1: 4625 (Failed Login) Ã— 20+ in 5 minutes (same source IP)
...
Step 2: If count > 20 â†’ mark IP as "brute force source"  
Step 3: Watch for successful login from same IP within 10 minutes
Step 4: If successful login â†’ escalate to HIGH severity
...
|-----------|---------------|
| Anomaly found â†’ escalate to SOC | â†’ Skill 11 (CSOC Automation) |
| Anomaly is a hunt lead | â†’ Skill 06 (Threat Hunting) |
| Build timeline from logs for IR | â†’ Skill 07 (Incident Response) |
| Create detection rules from findings | â†’ Skill 15 (Blue Team Defense) |

...
Enable Kiro to conduct comprehensive reconnaissance and open-source intelligence gathering during authorized security assessments. Kiro performs passive and active recon using its native analysis capabilities and orchestrates the included scripts for automation at scale.
...
- `nmap` â€” Active port scanning
- `amass` â€” Advanced subdomain enumeration
- `theHarvester` â€” Email and domain harvesting
...
1. **WHOIS Analysis** â€” Query domain registration records for registrant, registrar, nameservers, and dates. Flag privacy-protected registrations and registrar patterns.
2. **Certificate Transparency Logs** â€” Search crt.sh for all certificates issued to the domain and subdomains. Extract SANs (Subject Alternative Names) to discover hidden subdomains.
3. **DNS Records (Passive)** â€” Enumerate A, AAAA, MX, NS, TXT, SOA, SRV, and CNAME records using public resolvers. Analyze SPF, DKIM, and DMARC for email security posture.
4. **Search Engine Dorking** â€” Generate targeted dork queries to discover exposed files, login portals, and configuration leaks:
   - `site:target.com filetype:pdf` â€” Exposed documents
   - `site:target.com inurl:admin` â€” Admin panels
   - `site:target.com ext:env OR ext:config` â€” Config files
   - `"@target.com" site:linkedin.com` â€” Employee enumeration
   - `"target.com" site:pastebin.com` â€” Credential leaks
5. **Shodan/Censys Queries** â€” Search for internet-exposed services, open ports, banners, and vulnerabilities associated with the target's IP ranges.
6. **Git/Code Repository Search** â€” Search GitHub/GitLab for leaked credentials, API keys, and internal information:
...
1. **Certificate Transparency** â€” Extract all SANs from crt.sh/Censys certificates (most effective passive method)
2. **DNS Brute-Force** â€” Run subdomain_enum.py against the common wordlist in `resources/`
3. **Wildcard Detection** â€” Query random subdomains to detect wildcard DNS responses and filter false positives
4. **Resolution Validation** â€” Resolve all candidates to IP addresses; discard NXDOMAINs
5. **HTTP Probing** â€” Check which subdomains respond on ports 80/443; identify web applications
6. **Infrastructure Grouping** â€” Group discovered subdomains by IP/ASN to map cloud vs. on-prem assets
...
LIVE SUBDOMAINS:
  admin.example.com       â†’ 203.0.113.10  [HTTP 200] [nginx/1.18]
  dev.example.com         â†’ 203.0.113.11  [HTTP 302 â†’ /login]
  api.example.com         â†’ 203.0.113.12  [HTTP 200] [cloudflare]
  internal.example.com    â†’ 10.0.0.5      [No public response â€” internal?]

INFRASTRUCTURE CLUSTERS:
  203.0.113.10-15 â†’ AS12345 (Company Hosting)
  Cloudflare CDN â†’ 7 subdomains proxied
```
...
2. **Zone Transfer Attempt** (AXFR) â€” Try against all discovered nameservers:
   ```bash
   dig AXFR @ns1.example.com example.com
   ```
3. **Email Security Analysis:**
   - SPF: Check for `~all` (softfail) or `?all` (neutral) â€” both are weak
   - DMARC: Missing DMARC = zero enforcement; `p=none` = monitoring only
   - DKIM: Check selector existence and key strength
4. **Reverse DNS** â€” PTR lookups on all discovered IPs to find additional hostnames
5. **DNS History** â€” Check SecurityTrails or PassiveDNS for historical DNS records that may reveal old infrastructure

**Flag these misconfigurations:**
- Zone transfer allowed â†’ Exposes full DNS zone
- No DMARC record â†’ Email spoofing possible
- SPF `+all` â†’ Any server can send as this domain
- DNSSEC not configured â†’ DNS cache poisoning risk
...
1. Analyze HTTP response headers:
   - `Server:` â†’ Web server and version
   - `X-Powered-By:` â†’ Application framework
   - `Set-Cookie:` names â†’ Session framework (PHPSESSID=PHP, JSESSIONID=Java)
   - `X-Generator:` / `X-WordPress-Cache:` â†’ CMS
...
NOTABLE HEADERS:
  ...
  âœ“ Present: Strict-Transport-Security
```
...
```markdown
# Reconnaissance Report â€” [Target]
...
- Feed live web apps â†’ Skill 09 (Web Security)
- Feed discovered services â†’ Skill 02 (Vulnerability Scanner)
- Feed cloud assets â†’ Skill 10 (Cloud Security)
```
...
|-----------|-----------|--------------|
| Vulnerability assessment | Live services discovered | â†’ Skill 02 |
| Web application testing | Web apps found | â†’ Skill 09 |
| Cloud asset auditing | Cloud-hosted assets found | â†’ Skill 10 |
| Network traffic analysis | PCAP capture available | â†’ Skill 08 |
| IOC correlation | Suspicious infrastructure found | â†’ Skill 06 |
...
- [MITRE ATT&CK â€” Reconnaissance (TA0043)](https://attack.mitre.org/tactics/TA0043/)
- [Certificate Transparency â€” crt.sh](https://crt.sh/)

...
Enable Kiro to assist with reverse engineering tasks including binary analysis, assembly interpretation, decompilation, firmware reverse engineering, and protocol analysis. Kiro directly reads and interprets disassembled code, identifies patterns, reconstructs logic, and helps navigate complex binaries using RE tool output.
...
- `Ghidra` â€” NSA open-source RE framework (free)
- `radare2` / `Cutter` â€” Open-source RE framework
- `Binary Ninja` â€” Commercial RE platform with scripting
- `IDA Pro / Free` â€” Industry standard disassembler
- `GDB + GEF/PEDA/pwndbg` â€” Dynamic debugging
- `Binwalk` â€” Firmware extraction and analysis
- `strings, file, objdump, readelf` â€” Standard Linux utilities
...
Run these commands and share output with Kiro for analysis:
...
1. **Frame structure analysis** â€” Look for:
...
2. **Field type identification:**
   ...
   - 4 bytes, big-endian, values 0-65535 â†’ likely length or port
   - 16 bytes uniform random â†’ UUID or AES key
   - Null-terminated variable sequence â†’ ASCII string
   - Fixed 4 bytes: 0xDEADBEEF, 0xCAFEBABE â†’ magic number
   ```

3. **Command-response mapping** â€” Analyze pairs to find:
   - Request: specific type byte â†’ Response: matching acknowledgment
   - Error responses: common error code patterns

4. **State machine construction:**
   ```
   [INIT] â†’ send magic handshake â†’ [AUTH] â†’ send credentials â†’
   [CONNECTED] â†’ send commands â†’ [DATA] â†’ receive data â†’ [IDLE]
   ```
...
gdb ./challenge
# In GDB:
# info functions       â†’ list all functions
# disas main           â†’ disassemble main
# b *0x401234          â†’ breakpoint at address
# r < input.txt        â†’ run with input
```

**Common CTF patterns:**
- `gets()` / `scanf("%s")` without bounds â†’ stack buffer overflow
- `printf(user_input)` without format string â†’ format string vulnerability
- `strcmp(input, flag)` â†’ timing attack or direct comparison
- Custom cipher with key â†’ find key, XOR to decrypt
- VM-based challenge â†’ trace bytecode execution to find flag check

---

## Output Standards

When analyzing binaries, Kiro produces:
...
|-----------|---------------|
| Sample needs dynamic behavioral analysis | â†’ Skill 05 (Malware Analysis) |
| Vulnerability found â†’ develop exploit | â†’ Skill 03 (Exploit Development) |
| Extract IOCs from analysis | â†’ Skill 06 (Threat Hunting) |
| Create detection from findings | â†’ Skill 15 (Blue Team Defense) |
